...
class StageEditorState extends MusicBeatState
{
	...
	var charGrp:FlxSpriteGroup = new FlxSpriteGroup();
	...
	override function create()
	{
		...
		UI_stagebox.cameras = UI_box.cameras = [camHUD];

		UI_stagebox.selected_tab_id = 'Layers';

		var tipText:FlxText = new FlxText(FlxG.width - FlxG.width + 250, FlxG.height, 0, "Q/E - Camera Zoom In/Out
	...
	override public function update(elapsed:Float)
	{
		...
		{
			var mousePos:FlxPoint = FlxG.mouse.getScreenPosition(FlxG.camera);
	...
	{
		var int:Int = Std.int(layerStepper.value);
		visualLayers[int].screenCenter();
		visualLayers[int].x = visualLayers[Std.int(layerStepper.value)].x - 40 + FlxG.save.data.layerPositions[0];
		visualLayers[int].y -= 60 + FlxG.save.data.layerPositions[1];
	}
